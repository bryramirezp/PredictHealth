# /microservices\service-admins\app\core\security.py
# /microservices/service-admins/app/core/security.py
        ...
        try:
            ...
            if not all([user_id, user_type, email]):
                raise HTTPException(
                    ...
                    detail="Información de usuario no encontrada en headers. Use el API Gateway (backend-flask) en lugar de llamar el microservicio directamente. Endpoint correcto: http://localhost:5000/api/web/admin/...",
            ...
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="Error extrayendo información del usuario de headers. Asegúrese de usar el API Gateway (backend-flask) para acceder a los microservicios.",
                headers={"WWW-Authenticate": "Bearer"},
            )
...
async def get_current_user(request: Request) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Error obteniendo usuario actual. Use el API Gateway (backend-flask) para autenticación.",
            headers={"WWW-Authenticate": "Bearer"},
        )
...
def get_current_admin(current_user: Dict[str, Any] = Depends(require_user_type("admin"))):
    """
    Dependencia específica para obtener el administrador actual

    Args:
        current_user: Usuario actual validado

    Returns:
        Dict[str, Any]: Información del administrador
    """
    return current_user