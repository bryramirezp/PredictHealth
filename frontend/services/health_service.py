...
class HealthService:
    ...
    def get_dashboard_data(self, user_id: str, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            from backend.shared_models.prediction_algorithms import HealthRiskPredictor
            
            predictor = HealthRiskPredictor()
            
            # Predicción de diabetes
            diabetes_prediction = predictor.predict_diabetes_risk(user_data)
            
            # Predicción de hipertensión
            hypertension_prediction = predictor.predict_hypertension_risk(user_data)
            
            # Generar recomendaciones
            recommendations = predictor.generate_health_recommendations(
                [diabetes_prediction, hypertension_prediction], 
                user_data
            )
            ...
            dashboard_data = {
                ...
                "diabetesRisk": diabetes_prediction['puntuacion_riesgo'],
                "hypertensionRisk": hypertension_prediction['puntuacion_riesgo'],
                "factors": diabetes_prediction['factores_contribuyentes'][:3],
                "recommendations": recommendations[:3],
                "riskLevels": {
                    "diabetes": diabetes_prediction['nivel_riesgo'],
                    "hypertension": hypertension_prediction['nivel_riesgo']
                },
                "evolution": [
                    max(0, diabetes_prediction['puntuacion_riesgo'] - 10),
                    max(0, diabetes_prediction['puntuacion_riesgo'] - 5),
                    diabetes_prediction['puntuacion_riesgo'],
                    diabetes_prediction['puntuacion_riesgo'] + 2,
                    diabetes_prediction['puntuacion_riesgo'] + 5,
                    diabetes_prediction['puntuacion_riesgo']
                ],
                "distribution": [
                    diabetes_prediction['puntuacion_riesgo'],
                    100 - diabetes_prediction['puntuacion_riesgo'] - hypertension_prediction['puntuacion_riesgo'],
                    hypertension_prediction['puntuacion_riesgo']
                ]
            }
